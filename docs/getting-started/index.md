...
A typical first FlagStack setup has four steps:

1. run FlagStack or use a hosted instance;
...
For self-hosting, PostgreSQL 18 is the only required external service. The production FlagStack image contains the Go control plane, database migration command and compiled React dashboard.

## Recommended path

If you are evaluating FlagStack locally, follow these pages in order:
