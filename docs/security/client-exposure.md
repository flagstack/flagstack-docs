...
A client SDK key is not a secret. Security comes from controlling **what Switch On Your Code delivers**, not from trying to hide the key in a JavaScript bundle.
...
The official browser SDK rejects `syoc_server_...` credentials before making a request to help catch this mistake early.
